...
function insert(type) {
    ...
    switch (type) { 
        case 'tBox':
            typeIdentifier = 't'
            mainRoute = main[atIndex(eleToEditQ)].textarea
            break

        case 'cBox':
            typeIdentifier = 'i'
            mainRoute = main[atIndex(eleToEditQ)].checkbox
            break
            
        case 'rBut':
            typeIdentifier = 'r'
            mainRoute = main[atIndex(eleToEditQ)].radio
            break
        ...
        case 'userin':
            typeIdentifier = 'u'
            mainRoute = main[atIndex(eleToEditQ)].userin
            break
    }
    ...
    let existingEl = mainRoute.length
    if (typeIdentifier == 'o') { existingEl = 0 } //q{}o0
    ...
    if (typeIdentifier === 'o') {
        ...
        main[atIndex(eleToEditQ)].other = true
    ...
    main[atIndex(eleToEditQ)].display.push(`${typeIdentifier}${existingEl}`)
...
function viewForm() {
    ...
    saveForm().then(setTimeout(openNewWindow, 500))
}
...
function toggleOtherOp(q) {
    // console.log(main[atIndex(q)].other)
...
function toggleRequired(that) {
    ...
    main[atIndex(that.id.slice(1))].required = !main[that.id.slice(1)].required
}